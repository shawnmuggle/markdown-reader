...
- 📄 **本地文件直接渲染** —— 把 `.md` 拖进 Chrome，或打开 `file://` 路径的 Markdown 文件，自动渲染。本地 `.txt` 文件也会按 Markdown 渲染（超过 1 MB 的文件保留纯文本，保证流畅）
- 🎨 **GitHub 风格排版** —— 熟悉、干净的阅读体验
- 🧭 **侧边目录导航（TOC）** —— 自动根据标题生成，点击跳转，滚动高亮当前章节
- 🗂 **文件夹浏览** —— 打开本地文件夹（`file:///路径/目录/`）时，把 Chrome 那个简陋的原生目录页换成统一风格的文件列表，带面包屑和一键「返回上一级」
- 🔗 **同目录文件导航** —— 阅读某个 `.md` 时，左侧还会列出同一文件夹里的其它 Markdown 文件（当前文件高亮）以及返回上一级的入口，文件之间随手跳转，不用再敲路径
...
- 📦 **完全离线、100% 本地** —— 全程在浏览器里运行。没有远程服务器、没有后端、没有埋点。文件从本地磁盘读取、在本地渲染，**任何内容都不会被上传或外发**
...
这个插件**完全在本地运行**。读取和渲染文件的整个过程里**没有远程服务器、没有后端、没有任何网络请求**：

- 你的 Markdown 文件由**浏览器自己**从磁盘读取，**在页面内**渲染，内容从不离开你的机器。
- 文件夹列表和「同目录文件」侧栏都是**本地构建**的 —— 读取的是浏览器本就通过 `file://` 暴露的目录，不会把任何目录信息发到外部。
- 所有依赖库（marked、DOMPurify、highlight.js、mermaid、github-markdown-css）都以固定版本**打包在 `lib/`** 里，运行时不会从任何 CDN 拉取。
- 唯一存储的是你的**主题和语言偏好**，保存在你自己设备上的 Chrome 本地 `storage`。
- 唯一申请的权限是 `storage`（用于上述偏好）。插件**不申请**任何访问公开网站的主机权限。

一句话：装上、看你的本地文档，**什么都不会被上传**。完全可以离线使用。
...
│   ├── toc.js      # 侧边目录生成 + 滚动高亮
│   ├── siblings.js # 「同目录文件」侧栏导航（读取父目录）
│   ├── dirview.js  # file:///dir/ 目录页的统一风格文件列表
│   ├── theme.js    # 深浅色切换 + 偏好记忆
│   ├── i18n.js     # 界面语言（中/英）+ 偏好记忆
│   └── reader.css  # 布局样式（阅读视图 + 文件夹视图）
├── lib/            # vendored 第三方库
├── icons/          # 插件图标
├── test.md         # 功能验证样本
└── README.md
